...
export default function URLShortenerGuide({ pageData = {} }) {
  ...
  return (
    <>
      <Head>
        <title>{pageData.head?.title || 'URL 단축 가이드 - Marketing Tools'}</title>
        <meta name="description" content={pageData.head?.description || '긴 URL을 간단한 형태의 단축 링크로 변환하고 활용하는 방법을 알아보세요.'} />
        <meta name="keywords" content={pageData.head?.keywords || 'URL 단축, 단축 링크, 링크 관리, 공유 도구'} />
        <meta property="og:title" content={pageData.head?.ogTitle || 'URL 단축 가이드'} />
        <meta property="og:description" content={pageData.head?.ogDescription || '단축 URL 생성 및 활용 가이드'} />
...
export async function getStaticProps({ locale }) {
  ...
  return {
    props: {
      pageData: common.tools.url.urlGuide || {},
    },
  };
}
