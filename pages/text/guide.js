...
export default function TextCounterGuide({ pageData = {} }) {
  ...
  return (
    <>
      <Head>
        <title>{pageData.head?.title || '글자수 세기 가이드 - Marketing Tools'}</title>
        <meta name="description" content={pageData.head?.description || '텍스트 글자 수와 바이트를 효과적으로 분석하는 방법을 알아보세요.'} />
        <meta name="keywords" content={pageData.head?.keywords || '글자수 세기, 바이트 분석, UTF-8, EUC-KR'} />
        <meta property="og:title" content={pageData.head?.ogTitle || '글자수 세기 가이드'} />
        <meta property="og:description" content={pageData.head?.ogDescription || '글자수 세기 방법과 활용 가이드'} />
          ...
          <div className="bg-white/90 backdrop-blur border border-slate-100/80 rounded-[32px] p-8 mb-12 shadow-sm hover:shadow-[0_20px_40px_rgba(0,0,0,0.01)] transition-all duration-300">
            <div className="max-w-3xl">
              <div className="flex items-center space-x-3 mb-6">
                <Type className="w-10 h-10 text-blue-550" />
...
export async function getStaticProps({ locale }) {
  ...
  return {
    props: {
      pageData: common.tools.text.textGuide || {},
    },
  };
}
