...
export default function Guide({ pageData = {} }) {
  ...
  return (
    <>
      <Head>
        <title>{pageData.head?.title || '이미지 변환기 가이드 - Marketing Tools'}</title>
        <meta name="description" content={pageData.head?.description || '이미지 포맷을 손쉽게 대량 변환하는 방법을 알아보세요.'} />
        <meta name="keywords" content={pageData.head?.keywords || '이미지 변환, WEBP 변환, JPG 변환, PNG 변환'} />
        <meta property="og:title" content={pageData.head?.ogTitle || '이미지 변환기 가이드'} />
        <meta property="og:description" content={pageData.head?.ogDescription || '이미지 변환 방법과 활용 가이드'} />
...
export async function getStaticProps({ locale }) {
  ...
  return {
    props: {
      pageData: common.tools.imageConverter.imageConverterGuide || {},
    },
  };
}
