...
export default function YoutubeCommentsGuide({ pageData = {} }) {
  ...
  const FeatureCard = ({ icon, title, description }) => (
    <div className="bg-white/90 backdrop-blur border border-slate-100/80 rounded-[32px] p-6 shadow-sm hover:shadow-[0_20px_40px_rgba(0,0,0,0.01)] transition-all duration-300">
      <div className="flex items-start space-x-4">
        <div className="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0">{icon}</div>
        <div>
  ...
  return (
    <>
      <Head>
        <title>{pageData.head?.title || 'YouTube 댓글 추출기 가이드 - Marketing Tools'}</title>
...
export async function getStaticProps({ locale }) {
  ...
  return {
    props: {
      pageData: common.tools.youtubeComments.youtubeCommentsGuide || {},
    },
  };
}